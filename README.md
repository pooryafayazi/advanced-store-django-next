...
Docker can create the node_modules/ folder before create-next-app runs, which makes the directory not empty.
That can block the project creation.

Recommended workflow:

Generate the Next.js project first (with node_modules volume disabled).
...
Confirm it created (Windows PowerShell):
...
If the frontend container logs show next: not found, you need to install dependencies in the container/volume:
...
This can delete DB data if you are not using external volumes.
...
Fix: Do not publish Redis ports in dev (recommended), or change the published port.

2) npm ERR! enoent ... /usr/src/app/package.json
The Next project has not been created yet, or the folder is not mounted correctly.

Fix:

Ensure frontend/package.json exists.

Ensure compose mounts ../frontend:/usr/src/app.
...
If node_modules/ exists before running create-next-app, it can stop.
...
Temporarily disable the node_modules volume until after project creation.